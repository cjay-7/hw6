...
> create event "Meeting" from 2025-01-20T09:00 to 2025-01-20T10:00
...
**Description:** Executes commands from a text file, then opens the GUI with the loaded data. Useful for batch processing followed by interactive GUI editing.

**Example:**
```bash
# Process commands and then open GUI
java -jar build/libs/calendar.jar --mode headless-gui res/commands.txt
```
...
edit calendar --name <Name> name <NewName>
edit calendar --name <Name> timezone <NewTimezone>
```
...
**Recurring Events:**
```
create event "Subject" from yyyy-MM-ddTHH:mm to yyyy-MM-ddTHH:mm repeats <MTWRFSU> for <N> times
create event "Subject" from yyyy-MM-ddTHH:mm to yyyy-MM-ddTHH:mm repeats <MTWRFSU> until yyyy-MM-dd
create event "Subject" on yyyy-MM-dd repeats <MTWRFSU> for <N> times
create event "Subject" on yyyy-MM-dd repeats <MTWRFSU> until yyyy-MM-dd
```
...
edit events on yyyy-MM-dd set <property> "NewValue"
edit series <property> "Subject" on yyyy-MM-ddTHH:mm with "NewValue"
edit series <property> "Subject" from yyyy-MM-ddTHH:mm with "NewValue"
```

**Copying Events:**
```
copy event "Subject" on yyyy-MM-ddTHH:mm --target <CalendarName> to yyyy-MM-ddTHH:mm
copy events on yyyy-MM-dd --target <CalendarName> to yyyy-MM-dd
copy events from yyyy-MM-dd to yyyy-MM-dd --target <CalendarName> to yyyy-MM-dd
```
