...
public class EventTest {
  ...
  @Test
  public void testEventWithAllFields() {
    UUID eventId = UUID.randomUUID();
    UUID seriesId = UUID.randomUUID();
    Event event = new Event("Full Event",
        LocalDateTime.of(2025, 6, 15, 10, 0),
        LocalDateTime.of(2025, 6, 15, 11, 0),
        "Description here", "Room 101", true, eventId, seriesId);

    assertEquals("Full Event", event.getSubject());
    assertEquals("Description here", event.getDescription().get());
    assertEquals("Room 101", event.getLocation().get());
    assertTrue(event.isPrivate());
  ...
  @Test
  public void testEventWithModificationsPartial() {
    Event original = new Event("Original",
        LocalDateTime.of(2025, 6, 15, 10, 0),
        LocalDateTime.of(2025, 6, 15, 11, 0),
        "Desc", "Loc", false, UUID.randomUUID(), null);

    EventInterface modified = original.withModifications(
        "Only Subject Changed", null, null, null, null, null, null);
    assertEquals("Only Subject Changed", modified.getSubject());
    assertEquals("Desc", modified.getDescription().get());
    assertEquals("Loc", modified.getLocation().get());
    assertFalse(modified.isPrivate());
  }

  @Test
  public void testEventWithModificationsTimeChange() {
    Event original = new Event("Original",
        LocalDateTime.of(2025, 6, 15, 10, 0),
        LocalDateTime.of(2025, 6, 15, 11, 0),
        null, null, false, UUID.randomUUID(), null);
