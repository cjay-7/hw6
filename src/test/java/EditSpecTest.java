...
public class EditSpecTest {
  ...
  @Test
  public void testEditSpecBuilder() {
    ...
    assertEquals("Built Subject", spec.getNewSubject());
    assertEquals(start, spec.getNewStart());
    assertEquals(end, spec.getNewEnd());
    assertEquals("Built Desc", spec.getNewDescription());
    assertEquals("Built Loc", spec.getNewLocation());
    assertEquals(EventStatus.PRIVATE, spec.getNewStatus());
  }

  @Test
  public void testEditSpecBuilderPartial() {
    EditSpec spec = EditSpec.builder()
        .subject("Only Subject")
        .build();

    assertEquals("Only Subject", spec.getNewSubject());
    assertNull(spec.getNewStart());
    assertNull(spec.getNewEnd());
    assertNull(spec.getNewDescription());
    assertNull(spec.getNewLocation());
