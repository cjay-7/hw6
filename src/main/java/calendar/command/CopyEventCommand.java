...
public class CopyEventCommand implements CommandInterface {
  ...
  public boolean execute(CalendarManager manager, ViewInterface view) throws IOException {
    ...
    try {
      sourceStart = LocalDateTime.parse(sourceDateTime, DateTimeParser.DATETIME_FORMATTER);
    ...
    try {
      targetStart = LocalDateTime.parse(targetDateTime, DateTimeParser.DATETIME_FORMATTER);
    } catch (DateTimeParseException e) {
