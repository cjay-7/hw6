...
import calendar.util.TimezoneUtils;
...
public class CopyEventsRangeCommand implements CommandInterface {
  ...
  public boolean execute(CalendarManager manager, ViewInterface view) throws IOException {
    ...
    CalendarInterface sourceCalendar = manager.getCurrentCalendar();
    ...
    try {
      sourceStart = LocalDate.parse(startDate, DateTimeParser.DATE_FORMATTER);
      sourceEnd = LocalDate.parse(endDate, DateTimeParser.DATE_FORMATTER);
      targetStart = LocalDate.parse(targetStartDate, DateTimeParser.DATE_FORMATTER);
