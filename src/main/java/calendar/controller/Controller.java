 ...
 * The controller uses Readable instead of concrete types like System.in or
 * FileReader.
 * This demonstrates complete decoupling from the type of IO device - the
 * controller
 * can read from any Readable implementation (console, file, string, network,
 * etc.)
 * without modification. The abstraction allows for easier testing with
 * StringReader
...
public class Controller implements ControllerInterface {
   ...
   * @param interactive true for interactive mode (shows prompt), false for
   *                    headless
   */
    ...
    try (Scanner scanner = new Scanner(input)) {
      ...
      while (scanner.hasNextLine()) {
        ...
        try {
          ...
          view.displayError("Command failed: Invalid date/time format - " + e.getMessage());
